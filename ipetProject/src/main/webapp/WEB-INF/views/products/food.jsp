...
-->
    ...
    <section class="py-5">
        ...
        <div class="container px-4 px-lg-5 mt-5">
	        <h1 style="text-align: center;">사료/간식 판매</h1>
        ...
        <div class="pagination">
                <c:if test="${paging.prev}">
                  ...
                  <a href="/products/food?pageNum=${prevPage}">이전</a>
                </c:if>
                <c:forEach begin="${paging.startNum}"
                 ...
                 <c:choose>
                  <c:when test="${page ne paging.pageNum}">
						<a style="cursor:pointer;" class="pageClass" href="/products/food?pageNum=${page}">${page}</a>
				 	 </c:when>
				 	 <c:when test="${page eq paging.pageNum}">
				 	 	<a style="cursor:pointer; background-color: #cfe2f3;"class="pageClass" href="/products/food?pageNum=${page}">${page}</a>
				 	 </c:when>
				</c:choose>
                </c:forEach>
                <c:if test="${paging.next}">
                  <c:set var="nextPage" value="${paging.startNum+10}" />
                  <a href="/products/food?pageNum=${nextPage}" class="pageClass">다음</a>
                </c:if>
              </div>
            </div>
          	</section>
